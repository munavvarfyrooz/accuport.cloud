...
This module provides all database query functions for the Accuport dashboard.
It handles data retrieval from both the accubase.sqlite (measurement data, read-only)
and users.sqlite (user management, read-write) databases.

Key responsibilities:
- User authentication and vessel access control queries
- Vessel and sampling point data retrieval
- Measurement data queries with date filtering
- Alert management and generation
- Parameter limits lookup
"""
...
def get_user_by_username(username):
    ...
    Returns:
        dict: User data including id, username, password_hash, full_name,
              email, role, and is_active, or None if not found/inactive
    """
...
def get_user_by_id(user_id):
    """
    Retrieve an active user by their database ID.
...
def get_vessels_by_ids(vessel_ids):
    """
    Retrieve vessel details for a list of vessel IDs.

    Args:
        vessel_ids: List of integer vessel database IDs

    Returns:
        list: List of vessel dictionaries containing id, vessel_id (string code),
              vessel_name, email, and created_at, ordered alphabetically by name
    """
    if not vessel_ids:
        return []
...
def get_vessel_by_id(vessel_id):
    """
    Retrieve a single vessel by its database ID.

    Args:
        vessel_id: Integer database ID of the vessel

    Returns:
        dict: Vessel data including id, vessel_id (string code), vessel_name,
              email, and created_at, or None if not found
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def get_sampling_points_by_vessel(vessel_id):
    """
    Retrieve all active sampling points for a vessel.

    Sampling points represent equipment or locations where chemical tests
    are performed (e.g., boilers, engines, water systems).

    Args:
        vessel_id: Integer database ID of the vessel

    Returns:
        list: List of sampling point dictionaries with id, code, name,
              system_type, description, and is_active, ordered by code
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def get_sampling_point_by_code(vessel_id, code):
    """
    Retrieve a sampling point by its code for a specific vessel.

    Args:
        vessel_id: Integer database ID of the vessel
        code: Sampling point code (e.g., 'AB1', 'ME', 'PW1')

    Returns:
        dict: Sampling point data or None if not found
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def get_parameters():
    """
    Retrieve all test parameters defined in the system.

    Parameters represent the different chemical tests that can be performed
    (e.g., pH, Chloride, Phosphate, TBN).

    Returns:
        list: List of parameter dictionaries containing id, labcom_parameter_id,
              name, symbol, unit, ideal_low, ideal_high, category, and criticality
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def get_parameter_by_name(parameter_name):
    """
    Retrieve a parameter by name using partial matching.

    Args:
        parameter_name: Full or partial parameter name to search for

    Returns:
        dict: First matching parameter data or None if not found
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def get_latest_measurements_summary(vessel_id):
    """
    Get the most recent measurement for each parameter at each sampling point.

    Used for dashboard summary views to show current vessel status at a glance.

    Args:
        vessel_id: Integer database ID of the vessel

    Returns:
        list: List of dictionaries with sampling point info, parameter name,
              latest value, status, and measurement date
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def get_alerts_for_vessel(vessel_id, unresolved_only=True):
    """
    Retrieve alerts for a vessel, optionally filtering by resolution status.

    Alerts are generated when measurement values fall outside acceptable ranges.

    Args:
        vessel_id: Integer database ID of the vessel
        unresolved_only: If True, only return alerts that haven't been resolved

    Returns:
        list: List of alert dictionaries with alert details, parameter name,
              and sampling point name, ordered by alert date (newest first)
    """
    with get_accubase_connection() as conn:
        cursor = conn.cursor()
        query = '''
