...
A Flask-based web dashboard for marine chemical test solutions. Provides
role-based access control for vessel managers, fleet managers, and administrators
to view and analyze chemical test data from vessel water systems including
boiler water, engines, potable water, and more.

This module defines all Flask routes, authentication handlers, API endpoints,
and view functions for the Accuport dashboard application.
"""
# Application Version
APP_VERSION = "1.0.0"
...
def update_vessels_config_yaml(vessel_id, vessel_name, auth_token):
    """
    Update or add a vessel entry in the vessels_config.yaml file.

    If a vessel with matching vessel_id or vessel_name exists, updates its
    auth_token. Otherwise, creates a new vessel entry with default sampling points.

    Args:
        vessel_id: The internal vessel identifier (may be unused as ID is derived from name)
        vessel_name: Display name of the vessel (e.g., "M.V Racer")
        auth_token: Labcom API authentication token for the vessel
...
def inject_vessels():
    ...
    if hasattr(current_user, 'is_authenticated') and current_user.is_authenticated:
        ...
        return {'vessels': vessels, 'app_version': APP_VERSION}
    return {'vessels': [], 'app_version': APP_VERSION}
...
def dashboard():
    ...
    if selected_vessel_id:
        ...
        for alert in alerts:
            ...
            # Boiler
            elif any(keyword in sampling_point for keyword in ['BOILER', 'AB1', 'AB2', 'AUX BOILER', 'HOTWELL', 'EGE', 'HOT WELL']):
...
def boiler_water():
    ...
    alerts = [alert for alert in all_alerts
              if any(keyword in alert.get('sampling_point_name', '').upper()
                     for keyword in ['BOILER', 'AB1', 'AB2', 'AUX BOILER', 'HOTWELL', 'EGE', 'HOT WELL'])]
...
def admin_edit_vessel_details(vessel_id):
    ...
    details = get_vessel_details(vessel_id) or {}
    ...
    if request.method == 'POST':
        form_data = {
            ...
            'vessel_email': request.form.get('vessel_email', '').strip(),
            ...
            'bwtc1': request.form.get('bwtc1', '').strip(),
            'bwtc2': request.form.get('bwtc2', '').strip(),
            'bwtc3': request.form.get('bwtc3', '').strip(),
            'bwtc4': request.form.get('bwtc4', '').strip(),
            'bwtc5': request.form.get('bwtc5', '').strip(),
        ...
        # Handle vessel image upload
        if 'vessel_image' in request.files:
            file = request.files['vessel_image']
            if file and file.filename:
                import uuid
                ext = os.path.splitext(file.filename)[1].lower()
                if ext in ['.jpg', '.jpeg']:
                    filename = f"vessel_{vessel_id}_{uuid.uuid4().hex[:8]}{ext}"
                    filepath = os.path.join(app.root_path, 'static', 'vessel_images', filename)
                    try:
                        img = Image.open(file)
                        target_height = 228
                        ...
                        form_data['vessel_image'] = filename
                        # Delete old image
                        if details.get('vessel_image'):
                            old_path = os.path.join(app.root_path, 'static', 'vessel_images', details['vessel_image'])
...
def admin_dashboard():
    ...
    all_users = get_all_users_with_hierarchy()
...
@app.route("/api/admin/vessel-assignments/<int:user_id>")
@manager_required
def api_get_vessel_assignments(user_id):
    """Get vessels assigned to a user"""
    from flask import jsonify
    vessels = get_user_vessel_assignments(user_id)
...
def get_vessel_details_for_pdf(vessel_id):
    """
    Retrieve vessel details needed for PDF report cover page.

    Args:
        vessel_id: Integer ID of the vessel in the database

    Returns:
        tuple: (imo_number, company_name) or (None, None) if not found
    """
    try:
        with get_users_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT imo_number, company_name FROM vessel_details WHERE vessel_id = ?', (vessel_id,))
            row = cursor.fetchone()
            if row:
                return row['imo_number'], row['company_name']
    except Exception as e:
        app.logger.warning(f'Could not fetch vessel details: {e}')
    return None, None

@app.route('/api/reports/main-engine-sd-pdf')
@login_required
def api_main_engine_sd_pdf():
    """Generate PDF report for Main Engine Scavenge Drain page using main generator"""
    ...
    try:
        imo_number, company_name = get_vessel_details_for_pdf(vessel_id)
        pdf_bytes = generate_report_bytes(vessel_id, vessel_name, start_date, end_date, selected_sections=['main_engines'], imo_number=imo_number, company_name=company_name)
        filename = f"{vessel_name.replace(' ', '_')}_MainEngine_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.pdf"
        return send_file(io.BytesIO(pdf_bytes), mimetype='application/pdf', as_attachment=True, download_name=filename)
    except Exception as e:
        app.logger.error(f'Error generating Main Engine PDF: {str(e)}')
...
def api_boiler_water_pdf():
    ...
    try:
        imo_number, company_name = get_vessel_details_for_pdf(vessel_id)
        pdf_bytes = generate_report_bytes(vessel_id, vessel_name, start_date, end_date, selected_sections=['boiler'], imo_number=imo_number, company_name=company_name)
...
def api_aux_engines_pdf():
    ...
    try:
        imo_number, company_name = get_vessel_details_for_pdf(vessel_id)
        pdf_bytes = generate_report_bytes(vessel_id, vessel_name, start_date, end_date, selected_sections=['aux_engines'], imo_number=imo_number, company_name=company_name)
...
def api_potable_water_pdf():
    ...
    try:
        imo_number, company_name = get_vessel_details_for_pdf(vessel_id)
        pdf_bytes = generate_report_bytes(vessel_id, vessel_name, start_date, end_date, selected_sections=['potable_water'], imo_number=imo_number, company_name=company_name)
...
def api_central_cooling_pdf():
    ...
    try:
        imo_number, company_name = get_vessel_details_for_pdf(vessel_id)
        pdf_bytes = generate_report_bytes(vessel_id, vessel_name, start_date, end_date, selected_sections=['central_cooling'], imo_number=imo_number, company_name=company_name)
...
def api_treated_sewage_pdf():
    ...
    try:
        imo_number, company_name = get_vessel_details_for_pdf(vessel_id)
        pdf_bytes = generate_report_bytes(vessel_id, vessel_name, start_date, end_date, selected_sections=['treated_sewage'], imo_number=imo_number, company_name=company_name)
...
def run_sync_command(vessel_str_id, days='1825'):
    """
    Execute the data fetcher sync script for a specific vessel.

    Runs the fetch_and_store.py script as a subprocess to synchronize
    vessel measurement data from the Labcom API.

    Args:
        vessel_str_id: String identifier for the vessel (e.g., "mv_racer")
        days: Number of days of historical data to fetch (default: 1825 = ~5 years)

    Returns:
        tuple: (success: bool, output: str) where output is stdout on success
...
@app.route('/api/vessel-image/<int:vessel_id>')
@login_required
def api_vessel_image(vessel_id):
    """API endpoint to get vessel image URL"""
    from vessel_details_models import get_vessel_details
    details = get_vessel_details(vessel_id)
    if details and details.get('vessel_image'):
        return jsonify({
            'image_url': url_for('static', filename='vessel_images/' + details['vessel_image'])
        })
