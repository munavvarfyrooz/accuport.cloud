...
def get_all_users_with_hierarchy():
    """
    Get all users with fleet manager info for vessel managers
    Returns:
        list of user dicts with fleet_manager_name for vessel managers
    """
    with get_users_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def generate_auth_token():
    """
    Generate a secure random authentication token for vessel API access.

    Creates a URL-safe token prefixed with 'acc_' for easy identification
    of Accuport tokens.

    Returns:
        str: A 32-byte URL-safe random token prefixed with 'acc_'
    """
...
def get_all_vessels():
    """
    Retrieve all vessels from the accubase database.

    Returns:
        list: List of vessel dictionaries containing id, vessel_id, vessel_name,
              email, and created_at fields, ordered alphabetically by vessel_name.
    """
...
def get_all_vessels_with_tokens():
    """
    Retrieve all vessels with their associated authentication tokens.

    Fetches vessels from accubase.sqlite and merges with auth tokens
    from users.sqlite for a complete vessel configuration view.

    Returns:
        list: List of vessel dictionaries with additional auth_token and
              token_created_at fields for each vessel.
    """
...
def get_user_by_username(username):
    ...
    Used primarily for password reset flows where the user provides
    their username to identify their account.

    Args:
        username: The unique username to search for

    Returns:
        dict: User information (id, username, email, full_name, role, is_active)
...
def get_user_by_id(user_id):
    ...
    Used primarily for admin edit pages where the user ID is known
    from URL parameters.
...
def update_user(user_id, full_name, email, role, admin_user_id):
    ...
    Updates the user's full name, email, and role. All changes are
    logged to the audit log for tracking.

    Args:
        user_id: Integer database ID of the user to update
        full_name: New full name for the user
        email: New email address for the user
        role: New role ('vessel_manager', 'fleet_manager', or 'admin')
        admin_user_id: ID of the admin performing the update (for audit log)
    ...
    try:
        with get_users_connection() as conn:
            ...
            cursor.execute('''
                INSERT INTO admin_audit_log (admin_user_id, action_type, action_details, target_user_id)
                VALUES (?, 'update_user', ?, ?)
            ''', (admin_user_id, f'Updated user details: full_name={full_name}, email={email}, role={role}', user_id))
...
def reset_user_password(user_id, new_password, admin_user_id=None):
    ...
    Hashes the new password and updates the database. If admin_user_id is
    provided, it's used for the audit log; otherwise, the user's own ID is used
    (for self-service password resets).

    Args:
        user_id: Integer database ID of the user whose password is being reset
        new_password: Plain text new password (will be hashed before storage)
        admin_user_id: Optional ID of admin performing reset (for audit log)
